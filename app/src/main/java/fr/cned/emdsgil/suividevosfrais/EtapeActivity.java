...
public class EtapeActivity extends AppCompatActivity {
     ...
     */
    private void initListesVisiteur() {
    ...
    private void valoriseProprietes() {
        Integer annee = ((DatePicker)findViewById(R.id.datEtape)).getYear() ;
        Integer mois = ((DatePicker)findViewById(R.id.datEtape)).getMonth() + 1 ;
        ...
        findViewById(R.id.cmdEtapePlus).setOnClickListener(new Button.OnClickListener() {
            public void onClick(View v) {
                if (ficheEnCours.getEtat().equals("CR") | ficheEnCours.getEtat().equals("")){
                    // quantité limitée au nombre de jour du mois
                    YearMonth yearMonthObject = YearMonth.of(Integer.parseInt(anneeMois.substring(1,4)), Integer.parseInt(anneeMois.substring(5)));
                    Integer maxQte = yearMonthObject.lengthOfMonth();
                    qte = Math.min(maxQte, qte + 1);
                    ((EditText) findViewById(R.id.txtEtape)).setText(qte.toString());
                }else {
                    Toast.makeText(EtapeActivity.this, "Saisie impossible : fiche clôturée", Toast.LENGTH_SHORT).show();
                }
            }
        }) ;
    }
    ...
    private void dat_clic() {
        ...
        uneDate.init(uneDate.getYear(), uneDate.getMonth(), uneDate.getDayOfMonth(), new DatePicker.OnDateChangedListener(){
            ...
            public void onDateChanged(DatePicker view, int year, int monthOfYear, int dayOfMonth) {
                ...
                ((Button)findViewById(R.id.cmdEtapeValider)).setEnabled(ficheEnCours.getEtat().equals("CR")|ficheEnCours.getEtat().equals(""));
            }
        });
    }
